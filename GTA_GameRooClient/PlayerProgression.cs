    ...
    public class PlayerProgression : BaseScript {
        ...
        private void OnReceiveProgression( string json ) {
            try {
                ...
                bool isNewPlayer = dict.ContainsKey( "isNewPlayer" ) && Convert.ToBoolean( dict["isNewPlayer"] );

                dataLoaded = true;

                // Auto-open hub for first-time players
                if( isNewPlayer ) {
                    HubNUI.OpenHub( "", true );
                }
                ...
                Debug.WriteLine( "[GameRoo] Error parsing progression: " + ex.Message );
            }
        }
        ...
        public static async void StartCustomization() {
            ...
            Debug.WriteLine( "[GameRoo] StartCustomization: model=" + modelName + " hash=" + hash + " pos=" + _previewPos );
            RequestModel( hash );
            int timeout = 0;
            ...
            if( HasModelLoaded( hash ) ) {
                ...
                Debug.WriteLine( "[GameRoo] StartCustomization: ped=" + _previewPed + " exists=" + DoesEntityExist( _previewPed ) );
                ...
                Debug.WriteLine( "[GameRoo] StartCustomization: FAILED to load model after " + timeout + " ticks" );
            }
        }
        ...
        public static void ApplyFullAppearance( int ped, string json ) {
            ...
            try {
                ...
                Debug.WriteLine( "[GameRoo] Error applying appearance: " + ex.Message );
            }
        }
        ...
        private async void OnNuiCustomizeStart( IDictionary<string, object> data, CallbackDelegate cb ) {
            try {
                ...
                string modelName = string.IsNullOrEmpty( SelectedModel ) ? "mp_m_freemode_01" : SelectedModel;
                Debug.WriteLine( "[GameRoo] customizeStart: model=" + modelName + " _customizing=" + _customizing + " _previewPed=" + _previewPed );
                StartCustomization();
                ...
                Debug.WriteLine( "[GameRoo] customizeStart: OK, isFreemode=" + IsFreemodeModelName( modelName ) );
                cb( "{\"status\":\"ok\",\"settings\":" + settings + ",\"appearance\":" + appearance + "}" );
            } catch( Exception ex ) {
                Debug.WriteLine( "[GameRoo] customizeStart EXCEPTION: " + ex.Message );
        ...
        private async void OnNuiChangeModel( IDictionary<string, object> data, CallbackDelegate cb ) {
            string modelHash = data.ContainsKey( "modelHash" ) ? data["modelHash"].ToString() : "";
            Debug.WriteLine( "[GameRoo] changeModel: hash=" + modelHash + " _previewPos=" + _previewPos );
            ...
            try {
                ...
                if( !HasModelLoaded( hash ) ) {
                    Debug.WriteLine( "[GameRoo] changeModel: model failed to load after " + timeout + " ticks" );
                ...
                Debug.WriteLine( "[GameRoo] changeModel: created ped " + _previewPed + " exists=" + DoesEntityExist( _previewPed ) );
                ...
                Debug.WriteLine( "[GameRoo] changeModel: OK, isFreemode=" + IsFreemodeModelName( modelHash ) );
                cb( "{\"status\":\"ok\",\"settings\":" + settings + ",\"appearance\":" + appearance + "}" );
            } catch( Exception ex ) {
                Debug.WriteLine( "[GameRoo] changeModel EXCEPTION: " + ex.Message + "\n" + ex.StackTrace );
                cb( "{\"status\":\"error\",\"reason\":\"exception\"}" );
            }
        }
